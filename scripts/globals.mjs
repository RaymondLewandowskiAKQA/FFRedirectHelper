...
export class AEMUrl {
  ...
  set query(value) {
    ...
    if (typeof value === "string" || value instanceof String) {
      ...
      throw TypeError(
        "query must be set to either a query string or an object represeting one"
      );
    }
    return this._query;
  }
  ...
  isSubpageOf(url) {
    ...
    return (
      parsedUrl.domain === this.domain &&
      trimExt(basePath).startsWith(trimExt(urlPath))
    );
  }
  ...
  getRelativePath(baseUrl) {
    ...
    function isNotInBaseUrl(basePath) {
      ...
      var fragments = basePath
        ? AEMUrl.prototype.getPathSegments(basePath)
        : [];
  ...
  changeEnvironment(oldEnv, newEnv) {
    ...
    if (
      parsedNewEnv.path === AUTHOR_PATH ||
      parsedNewEnv.path === SITEADMIN_PATH
    ) {
      ...
      newUrlObj.path =
        parsedNewEnv.path.endsWith("/") || !newPath
          ? [parsedNewEnv.path, newPath].join("")
          : [parsedNewEnv.path, newPath].join("/");
      if (this.path !== AUTHOR_PATH && this.path !== SITEADMIN_PATH) {
  ...
  isSiteAdmin() {
    return this.protocol && this.domain && this.path === SITEADMIN_PATH;
  }
  ...
  isPublished() {
    return (
      this.protocol && this.domain && !(this.isPreview() || this.isAuthor())
    ); // this could be optimized, but this is clearer
  }
  ...
  setAuthor(toAuthor = true) {
    ...
    if (toAuthor) {
      this.fragment =
        this.path === AUTHOR_PATH || this.path === SITEADMIN_PATH
          ? this.fragment
          : this.path;
