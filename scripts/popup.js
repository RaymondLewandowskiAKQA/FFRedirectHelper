...
var BUTTON_MAP = {
  ...
  // regardless of which button opened the dropdown)
    openInNewTab:{
      ...
      events: {
        click: function(event, {currentSite,targetSite,selectedTabs,parentName}) {
          var updateProperties={},
            destUrl;
          switch (parentName) {
            case "siteadmin":
              console.log("input",targetSite.author);
              destUrl = new globals.AEMUrl(targetSite.author).setSiteAdmin(true);
              break;
            case "author":
          ...
          if (selectedTabs.length>1) {
            if (!window.confirm("Are you sure you want to open "+selectedTabs.length+" new tabs?")){
              return;
            }
            updateProperties.active=false;
          }
          selectedTabs.reverse().forEach((tab)=>{
            var url = changeEnvironment(
              tab.url,
              currentSite,
              destUrl
            );
...
function renderCard(targetSite,currentSite,selectedTabs) {
  ...
  Object.entries(BUTTON_MAP).forEach(([name,data])=>{
    if (name !== DROPDOWN_KEY) {
      buttonContainer.appendChild(renderButton(data,{
        card:template,
...
.then(([currentTab,selectedTabs])=>{
  ...
  .then((data)=>{
    ...
    for (currentSite of data.sites) {
      if (tabAemUrl.isSubpageOf(currentSite.author) || 
    ...
    for (const targetSite of data.sites) {
      document.getElementById(CONTAINER_ID).appendChild(
        renderCard(targetSite,currentSite,selectedTabs)
      );
    }
    setActiveState(document.getElementById(ENV_LABEL_ID),
      data.showEnvLabels);
    setActiveState(document.getElementById(TOGGLE_OUTLINE_ID),
  ...
  document.getElementById(TOGGLE_OUTLINE_ID).addEventListener("click",(evt)=>{
    toggleOutline(evt,evt.currentTarget,currentTab);
  });
  document.getElementById(DISABLE_CACHE_ID).addEventListener("click",(evt)=>{
