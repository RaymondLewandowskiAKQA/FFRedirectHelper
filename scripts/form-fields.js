...
export function assertInputType(ele,expected) {
  if (expected instanceof Array) {
    return ele.type && expected.some((expVal)=>{return ele.type===expVal})
...
export class CheckboxField extends Field {
  ...
  fromInput(elementArray) {
    ...
    return {
      name:this.name,
      value:this.combineValues?checkboxValues.join(ITEM_DELIM) : checkboxValues
    };    
  }
...
const EXPORT_FIELD_VALIDATION = {
  ...
  "sites":[{
    "id":validateValue((val)=>{return typeof val==="number" && val%1===0 && val>=0}),
    "color":validateRegex(/^#(?:[0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/,"'Color' must be a hexadecimal color code"),
    ...
    "contextMenu":validateCheckBoxes(/^author|preview|published$/,"Invalid checkbox value")
  }]
};

export function sanitiseJsonSettings(jsonData,cleanObj,validationMap=EXPORT_FIELD_VALIDATION) {
    ...
    if (validationMap.hasOwnProperty(key)) {
      if (validationMap[key].constructor === Function) {
        ...
        if (Array.isArray(validationMap[key])) {
          console.assert(validationMap[key].length===1,"Only one type is allowed in an array");
          cleanObj[key]=[];
          ...
          cleanObj[key]={};
          sanitiseJsonSettings(jsonData[key],cleanObj[key],validationMap[key])
        }
